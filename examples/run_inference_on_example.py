...
def rendering(predictions, dataset_dir, renderer):
...
def run_inference(dataset_dir: Path, dataset_to_use: str) -> None:
    ...
    for i in range(len(img_names)):
        ...
        renderings = rendering(predictions, dataset_dir, renderer)
        save_prediction_img(dataset_dir / "output", img_name, rgb, renderings.rgb)
        all_predictions.append(predictions_to_dict(predictions))

        print(f"({i+1}/{len(img_names)}) inference successfully. {(time.time() - frame_processing_time_start):9.4f}s")
