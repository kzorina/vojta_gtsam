...
def main():
    ...
    for i in reversed(range(0, 10)):  # odometry
        T_bc = rnd_T()
        new_graph.add(gtsam.PriorFactorPose3(X(i), T_bc, noise))
        initial_estimate.insert(X(i), T_bc)
        new_graph.add(gtsam.BetweenFactorPose3(X(i), L(0), rnd_T(), noise))
        # new_graph.add(gtsam.BetweenFactorPose3(L(0), X(i), rnd_T(), noise))
