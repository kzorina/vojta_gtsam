...
T_cn_pin:pin.SE3 = pin.SE3.Random()   # estimated body pose in world frame
T_wo_pin:pin.SE3 = pin.SE3.Random()   # estimated body pose in world frame

Q_wc_pin:np.ndarray = random_cov()
Q_cn_pin:np.ndarray = random_cov()
...
def f_pin(T_wc:pin.SE3, T_cn:pin.SE3, T_wo:pin.SE3):
    return T_wo.inverse() * T_wc * T_cn
...
T_on_pin:pin.SE3 = f_pin(T_wc_pin, T_cn_pin, T_wo_pin)
...
J_on_wc_pin = T_cn_pin.inverse().action
J_on_cn_pin = np.eye(6)
Q_on_pin = J_on_wc_pin @ Q_wc_pin @ J_on_wc_pin.T + J_on_cn_pin @ Q_cn_pin @ J_on_cn_pin.T
...
print(frobenius_norm(Q_on_num, Q_on_pin))
